...
Because there are many nuances associated with predicting weather a single variable would not suffice in order to predict the high temperature on Monday. As a result I created a variable based off of the the average high temperature, temperature, pressure, and humidity over the course of the course of September 16-18 from the years 2011-2020. Each of these values were normalized using the function below:

![](C:/Users/brand/Documents/Statistics-Notebook/Analyses/Linear Regression/img1.jpg)
...
Once the values were re-scaled a weighted mean was performed with the weights being a fraction of 1 over the p-value of the linear model p-value of the re-scaled variable. The formula looks as follows with A, B, C... being the feature variables:

![](C:/Users/brand/Documents/Statistics-Notebook/Analyses/Linear Regression/img2.jpg)
...
The following was the output of the linear regression as well as the model:
...
$$
  \underbrace{\hat{Y}_i}_\text{High Temperature} = 55.85 + 1.382 \underbrace{X_i}_\text{Predictor Variable}
$$
...
Assuming that this is an appropriate model, as the predictor variable increases by one unit, the average high temperature will increase by 1.38 degrees Fahrenheit. Because the predictor variable is unlikely to have a value of 0 because the values weighted are unlikely to reach 0, the p-value for b0 is not particularly helpful. 

### Validation of the Model

After checking the appropriateness of the model, there appears to be a couple of major concerns. First, there are some major outliers with points 5, 9, and 10. This makes the spread on the residuals vs fitted plots not uniform. It is also questionable if the data could be considered normal, because the outliers fall outside of the bounds of the qq-plot.
